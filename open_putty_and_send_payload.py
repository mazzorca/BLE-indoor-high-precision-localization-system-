...
def OpenPutty(path, host, user='bicadmin', pwd='manage', pos=[150, 100, 12000, 800]):
    ...
    p = Popen(["putty.exe", '%s@%s' % (user, host), '-pw', pwd],
              creationflags=CREATE_NEW_CONSOLE)  # , #, env=my_env) #stderr=PIPE, stdout=PIPE,
...
if __name__ == '__main__':
    ...
    pld = ['ls -al\n']
    puttywin = OpenPuttyAndSendPayload(pld=pld, title='Putty test', user='bicadmin', pwd='bicadmin',
                                       host='ny5lsctgbiuniv1', pos=pos)
